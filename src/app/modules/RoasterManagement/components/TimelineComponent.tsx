    ...
    const itemRenderer = ({ item, timelineContext, itemContext, getItemProps, getResizeProps }): JSX.Element => {
        ...
        return (
            <Popover
                ...
                // position="right"
                // headerContent={<div>{item.title}</div>}
                // bodyContent={hide => (
                //     <div>
                //         <Divider />
                //         <List isPlain iconSize="large" style={{ padding: '15px' }}>
                //             <ListItem icon={<OutlinedCalendarAltIcon />}>{`${moment(item.start, "x").format("MMM D, YYYY")} - ${moment(item.end, "x").format("MMM D, YYYY")}`}</ListItem>
                //             <ListItem icon={<ServerGroupIcon />}>{item.description}</ListItem>
                //             <ListItem icon={<UsersIcon />}>{item.mail}</ListItem>
                //         </List>
                //     </div>
                // )}
                // footerContent={(hide) => (
                //     <ActionList>
                //         <ActionListItem>
                //             <Button variant="secondary" onClick={() => handleEditEvent(hide, item)} isSmall>Edit</Button>{' '}

                //         </ActionListItem>
                //         <ActionListItem>
                //             <Button variant="secondary" isDanger onClick={() => handleDeleteClick(hide, item)} isSmall>Delete</Button>{' '}
                //         </ActionListItem>
                //     </ActionList>
                // )}
                id={id}
